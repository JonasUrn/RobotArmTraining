...
class LogCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if time.time() - self.start > self.time_budget:
            ...
            return False
        if len(self.ep_rewards) > 0 and len(self.ep_rewards) % 25 == 0:
            n = min(25, len(self.ep_rewards))
            mr = np.mean(self.ep_rewards[-n:])
            sr = np.mean(self.ep_success[-n:])
            print(f"steps={self.num_timesteps} eps={len(self.ep_rewards)} mean_r={mr:.2f} succ={sr:.2f}")
        return True
...
def main():
    ...
    cb = LogCallback(TIME_BUDGET_SEC)
    t0 = time.time()
